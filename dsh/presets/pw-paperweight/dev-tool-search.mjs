/**
 * dev-tool-search — on-demand tool discovery and unlock for router-flash.
 *
 * Mechanism borrowed from xiaobright/dsh-anchored-standard (dev-tool-search.mjs),
 * adapted to this preset's catalog. The promoted phase keeps only a minimal
 * resident set (persistent bash + str_replace_editor + this tool) instead of
 * dumping the whole Standard catalog at once. This plugin registers ONE small
 * tool:
 *
 ...
 *    durable `tool/call` arguments, and anchored-bootstrap.mjs's assemble
 *    filter exposes them from the next request on (resume-safe).
 *
...
/**
 * The capability index: the resident minimal tools (bash / str_replace_editor)
 * cannot cover these, so the model must search and unlock them on demand.
 * Kept in the description so the model KNOWS what exists without a full
 * catalog dump.
 */
const UNLOCKABLE_INDEX = [
  'read / write / edit / glob / grep — sandboxed filesystem tools',
  ...
  'skill — load a skill from the catalog',
]
...
export function apply(ctx) {
  ctx.tools.register({
    ...
    description: [
      ...
      '',
      'This session starts with a minimal resident set: bash and str_replace_editor. Everything else is unlocked on demand through this tool.',
      '',
    ...
    parameters: toJsonSchema({
      ...
      toolNames: { type: 'array', required: false, description: 'exact tool names to unlock', items: 'string' },
    }),
    output: {
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        // scope only sees the global layer.
