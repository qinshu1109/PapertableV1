...
### 2. 写稿：提取主张 + 经典套壳（拒绝原创）+ 双稿二选一

先把 `pack.json` 完整读一遍。写作时遵守下面的证据白名单协议与三大常青爆款壳。

**三大常青爆款壳（禁止从零发明表达）：**
- **壳 A【反常识打脸体】**（适用于速记与常识/官方声明冲突）：
  - 骨架：反常识断言钩子（配额没超却报429）→ 现场踩坑实录 [N#] → 外部证据打脸实锤 [E#] → 深度机制解密 [E#] → 犀利主张（「我的看法：」）。
- **壳 B【SRE 排障复盘体】**（适用于一手故障与排查细节丰富）：
  - 骨架：事故现象 [E#] → 现场切入排查 [N#] → 社区同类复现与无根因现象 [E#] → 架构根因与跨池解法 [E#] → 架构防范主张（「我的看法：」）。
- **壳 C【避坑清单体】**（适用于同主题速记 ≥3 条）：
  - 骨架：总括踩坑数 → 坑点 1/2/3 连环展开 [N#] + 对应证据实锤 [E#] → 总结避坑口诀（「我的看法：」）。

**核心纪律：双稿二选一（人只翻牌子，不当编辑）**
- 每次写稿必须产出 **Version 1（倾向打脸/吐槽）** 与 **Version 2（倾向工程/复盘）** 两篇草稿（分别保存为 `/tmp/draft1.md` 与 `/tmp/draft2.md`）；
- 两篇均须通过 `verify_draft.mjs` 验真；
- 同时摆在用户面前，让用户在 3 秒内做出二选一（或直接贴去发布）。
