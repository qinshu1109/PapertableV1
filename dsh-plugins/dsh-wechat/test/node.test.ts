...
let createdHeaders: Array<{ id: string; cwd?: string }>
...
const factory: AgentFactory = {
  async createAgent(ownerCtx, options) {
    ...
    createdHeaders.push({ id: session.id, ...session.header.cwd === undefined ? {} : { cwd: session.header.cwd } })
...
test('/new defaults the session cwd to the host process cwd ({{cwd}} must resolve)', async () => {
  await mountNode()
  const before = createdSessions.length
  server.enqueue(textMessage('/new 无 cwd 配置时兜底'))
  await waitFor(() => createdSessions.length === before + 1, 3000)
  const header = createdHeaders.at(-1)!
  assert.equal(header.id, createdSessions.at(-1))
  assert.equal(header.cwd, process.cwd(), 'session must carry an absolute cwd so the harness {{cwd}} prompt variable resolves')
})

test('/new honors a configured cwd', async () => {
  await mountNode({ cwd: '/tmp/wechat-work' })
  const before = createdSessions.length
  server.enqueue(textMessage('/new 指定 cwd'))
  await waitFor(() => createdSessions.length === before + 1, 3000)
  assert.equal(createdHeaders.at(-1)!.cwd, '/tmp/wechat-work')
})
