...
plugins:
  dsh-chatnode-wechat:
    ...
    # cwd: /path/to/workspace         # working dir for `/new` sessions (defaults to the host process cwd; a session without an absolute cwd fails the harness `{{cwd}}` prompt variable)
